...
static inline void MuLawCompress(int16_t* sample, int8_t* output)
{
    ...
    // If the entire vector is below the zero threshold, exit early
    uint16x8_t below_zero_vec = vcgtq_s16(in, zero);
    uint64x2_t below_64 = vreinterpretq_u64_u16(below_zero_vec);
    uint8_t gt0 = vgetq_lane_u64(below_64, 0) | vgetq_lane_u64(below_64, 1);
    if(!gt0)
    {
        return;
    }
...
static inline void MuLawDecompress(int8_t* sample, int16_t* output)
{
    ...
    // If the entire input is zero, exit early.
    // Do this by casting input as unsigned 64-bit value, check for all 1s
    int64_t zero_check = (int64_t*)sample[0];
    if(zero_check == 0xFFFFFFFF)
        return;
